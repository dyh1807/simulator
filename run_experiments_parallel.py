...
def run_simulation(task):
    ...
    try:
        ...
        result = subprocess.run([f"./{binary}", image], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=3000)
        end_time = time.time()
...
def main():
    ...
    for r in results:
        ...
        if "error" in r:
            print(f"{img_name:<15} | {latency:<5} | Error: {r['error']}")
        else:
            ...
            print(f"{img_name:<15} | {latency:<5} | {inst_num:<10} | {cycles:<10} | {ipc_str:<8} | {access:<10} | {miss:<8} | {icache_acc_str:<10} | {bpu_acc_str:<8} | {elapsed_str:<7}")
    print("="*len(header))
