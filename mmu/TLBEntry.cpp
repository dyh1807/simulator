...
void TLBEntry::set_valid_pte(pte_t pte, uint32_t asid_in, uint32_t vpn1_in,
...
bool TLBEntry::is_page_fault(uint8_t op_type, uint32_t mstatus,
  ...
  if (read || execute) {
    if ((op_type == 0 && execute) || (op_type == 1 && read) ||
        (op_type == 1 && mxr && execute) || (op_type == 2 && write)) {
      ; // 允许的访问类型
    } else {
    ...
    // 检查 S 模式下的访问权限
    if (privilege == 1 && !sum_m && user && !sum_s)
      return true; // S 模式下不允许访问 U 模式可访问的页面

    if (privilege != 1 && mprv && mpp == 1 && !sum_m && user && !sum_s)
      return true; // 非 S 模式下，MPRV 启用且 MPP 为 S 模式时不允许访问 U
                   // 模式可访问的页面
...
void TLBEntry::log_entry() const {
  // 打印 TLB 条目的信息
  std::cout << "TLB Entry: "
            << "VPN1: " << std::hex << vpn1 << ", VPN0: " << vpn0
            << ", PPN1: " << ppn1 << ", PPN0: " << ppn0 << ", ASID: " << asid
            << ", Megapage: " << megapage << ", Dirty: " << dirty
            << ", Accessed: " << accessed << ", Global: " << global
            << ", User Visible: " << user << ", Execute: " << execute
            << ", Write: " << write << ", Read: " << read
            << ", Valid: " << valid << ", PTE_Valid: " << pte_valid
            << std::endl;
}
