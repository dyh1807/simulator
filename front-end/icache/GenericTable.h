...
public:
  ...
  void seq(const DynamicTableReadReq &read_req,
    ...
    if (read_req.enable && !pending_valid_ && storage_.valid_address(read_req.address)) {
      pending_valid_ = true;
...
private:
  ...
  uint32_t choose_latency() {
    uint32_t latency = generic_table_detail::clamp_latency(config_.fixed_latency);
    if (config_.random_delay) {
      uint32_t min_lat = generic_table_detail::clamp_latency(config_.random_min);
