...
def main() -> int:
    ...
    header_path = os.path.join(repo_root, "mmu", "include", "ptw_module.h")
    out_path = os.path.join(repo_root, "mmu", "include", "ptw_pi_po.h")
    map_prefix = os.path.join(repo_root, "mmu", "include", "ptw")

    pi_fields = [
        ("in", "PTW_in_t"),
        ("regs", "PTW_regs_t"),
        ("lookup_in", "PTW_lookup_in_t"),
    ]
    po_fields = [
        ("out", "PTW_out_t"),
        ("reg_write", "PTW_regs_t"),
        ("table_write", "PTW_table_write_t"),
    ]

    text = generate_header(
        header_path=header_path,
        include_path="ptw_module.h",
        ns="ptw_module_n",
        out_ns="ptw_pi_po",
        wrapper_type="PTW_IO_t",
        pi_fields=pi_fields,
        po_fields=po_fields,
        type_overrides={},
    )
